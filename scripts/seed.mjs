...
`;
  ...
  // The login screens show a "use this demo account" helper. The owner and partner emails carry a
  // per-run suffix, so the values are published here rather than hardcoded — a stale hardcoded
  // address would be worse than showing nothing.
  //
  // The apps live in their own repositories now, so each one is written only if it is actually
  // checked out somewhere this script can see: beside the backend, or as a sibling directory.
  const demoEnv = {
    'admin-portal': [
      `NEXT_PUBLIC_DEMO_EMAIL=${ADMIN.email}`,
      `NEXT_PUBLIC_DEMO_PASSWORD=${ADMIN.password}`,
      `NEXT_PUBLIC_DEMO_ALT_EMAIL=${OPS.email}`,
      `NEXT_PUBLIC_DEMO_ALT_PASSWORD=${OPS.password}`,
    ],
    'owner-web': [
      `NEXT_PUBLIC_DEMO_EMAIL=${OWNER.email}`,
      `NEXT_PUBLIC_DEMO_PASSWORD=${OWNER.password}`,
    ],
    'partner-console': [
      `NEXT_PUBLIC_DEMO_EMAIL=${PARTNER.email}`,
      `NEXT_PUBLIC_DEMO_PASSWORD=${PARTNER.password}`,
    ],
  };

  const candidates = (app) => [
    new URL(`../../apps/${app}/`, import.meta.url),
    new URL(`../../turfgang-${app}/`, import.meta.url),
    new URL(`../../../turfgang-repos/turfgang-${app}/`, import.meta.url),
  ];

  const written = [];
  for (const [app, lines] of Object.entries(demoEnv)) {
    for (const dir of candidates(app)) {
      if (!existsSync(dir)) continue;
      writeFileSync(
        new URL('.env.local', dir),
        `# Written by scripts/seed.mjs — demo sign-in helper. Not for production.\n${lines.join('\n')}\n`,
      );
      written.push(app);
      break;
    }
  }
  if (written.length > 0) log(`demo credentials written for ${written.join(', ')}`);

  // React Native cannot read .env.local, so the mobile app gets a generated module instead —
  // again, only when it is actually checked out somewhere reachable.
  for (const dir of [
    new URL('../../apps/owner-mobile/src/', import.meta.url),
    new URL('../../turfgang-owner-mobile/src/', import.meta.url),
    new URL('../../../turfgang-repos/turfgang-owner-mobile/src/', import.meta.url),
  ]) {
    if (!existsSync(dir)) continue;
    writeFileSync(
      new URL('demoCredentials.ts', dir),
      `/** Generated by scripts/seed.mjs. Shown on the login screen so the demo account is one tap away. */\n` +
        `export const DEMO_CREDENTIALS = {\n` +
        `  email: ${JSON.stringify(OWNER.email)},\n` +
        `  password: ${JSON.stringify(OWNER.password)},\n` +
        `} as const;\n`,
    );
    log('demo credentials written for owner-mobile');
    break;
  }
