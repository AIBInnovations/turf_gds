...
`;
  ...
  // The login screens show a "use this demo account" helper. The owner and partner emails carry
  // a per-run suffix, so the values are published here rather than hardcoded in the apps — a
  // stale hardcoded address would be worse than showing nothing.
  // Each app is its own repository, checked out as a sibling of the backend. `../..` from
  // scripts/ reaches that shared parent; an app that is not checked out is skipped rather than
  // failing the seed, because seeding the backend is useful on its own.
  const appDir = {
    'admin-portal': '../../turfgang-admin-portal',
    'owner-web': '../../turfgang-owner-web',
    'partner-console': '../../turfgang-partner-console',
  };
  ...
  for (const [app, lines] of Object.entries(envFiles)) {
    const target = new URL(`${appDir[app]}/.env.local`, import.meta.url);
    if (!existsSync(new URL(appDir[app], import.meta.url))) continue;
    writeFileSync(
      target,
  ...
  const mobileDir = new URL('../../turfgang-owner-mobile', import.meta.url);
  if (existsSync(mobileDir))
  writeFileSync(
    new URL('../../turfgang-owner-mobile/src/demoCredentials.ts', import.meta.url),
