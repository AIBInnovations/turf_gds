...
- Given a partial refund produces fractional component values, when reversals
  are posted, then rounding residuals are allocated deterministically and the
  journal remains balanced.
- Given a reversal references another Booking or financial scope, or repeats
  an already reversed entry, when posting is attempted, then it is rejected.
...
Priority: `P1`
Notes: Implemented through new evidenced `ADJUSTMENT` entries; historical
Ledger records and completed Settlements are never updated.
...
Priority: `P1`
Notes: Implemented under `/api/v1/partners/me/settlements`; customer invoices
are not generated by GDS.
