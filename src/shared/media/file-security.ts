...
export function inspectUploadedFile(
  ...
  if (buffer.length === 0)
    throw invalid('EMPTY_UPLOAD', 'Uploaded file is empty');
  const detected = detectMime(buffer);
  if (!detected || !allowed.includes(detected))
    throw invalid(
      'UNSUPPORTED_FILE_CONTENT',
      'The uploaded bytes are not an allowed file type',
    );
  if (detected !== claimedMime.toLowerCase())
    throw invalid(
      'FILE_CONTENT_MISMATCH',
      'The uploaded file content does not match its declared MIME type',
    );
  if (
    buffer.includes(Buffer.from('EICAR-STANDARD-ANTIVIRUS-TEST-FILE', 'ascii'))
  )
    throw invalid(
      'MALWARE_DETECTED',
