...
const partnerAccessRoutes: FastifyPluginAsync<
  PartnerAccessRoutesOptions
> = async (fastify, options) => {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      const partner = requirePartnerPortalContext(request);
      await options.service.logoutPortalSession(
        partner.partnerId,
  ...
  fastify.get(
    '/webhooks',
    { config: { rawBody: true }, preHandler: partnerAuth },
    async (request) => {
      const partner = requirePartnerScope(request, 'webhooks:write');
      return options.service.listWebhooks({
        partnerId: partner.partnerId,
        environment: partner.environment,
      });
    },
  ...
  fastify.post<{ Params: { webhookId: string } }>(
    '/webhooks/:webhookId/rotate-secret',
    {
      ...
      schema: {
        params: {
          ...
          required: ['webhookId'],
          properties: {
            webhookId: { type: 'string', pattern: '^[a-fA-F0-9]{24}$' },
          },
        },
      },
    },
    async (request) => {
      const partner = requirePartnerScope(request, 'webhooks:write');
      return options.service.rotateWebhookSecret({
  ...
  fastify.post<{ Params: { webhookId: string } }>(
    ...
    async (request) => {
      const partner = requirePartnerScope(request, 'webhooks:write');
      return options.service.testWebhook({
        webhookId: request.params.webhookId,
        partnerId: partner.partnerId,
        environment: partner.environment,
      });
