...
const kycRoutes: FastifyPluginAsync<KycRoutesOptions> = async (
  ...
  const ownerAuth = createOwnerAuthenticationHook(options.ownerAccessService);
  const adminAuth = createAdminAuthenticationHook(options.adminAuthService);
  if (options.partnerAccessService) {
    const partnerPortalAuth = createPartnerPortalAuthenticationHook(
    ...
    fastify.post<{
      ...
      '/partner/verifications/:verificationId/documents',
      {
        preHandler: partnerPortalAuth,
        schema: {
          params: {
            ...
            properties: {
              verificationId: { type: 'string', pattern: '^[a-fA-F0-9]{24}$' },
            },
          },
          querystring: {
            type: 'object',
      ...
      async (request, reply) => {
        ...
        return reply
          .status(201)
          .send(
            await options.service.uploadDocument({
    ...
    fastify.post<{ Params: { verificationId: string } }>(
      '/partner/verifications/:verificationId/submit',
      {
        preHandler: partnerPortalAuth,
        schema: {
          params: {
    ...
    fastify.get<{ Params: { verificationType: string } }>(
      '/partner/verifications/current/:verificationType',
      {
        preHandler: partnerPortalAuth,
        schema: {
          params: {
    ...
    fastify.patch<{
      Params: { verificationId: string; documentId: string };
      Body: { documentType: string; details: Record<string, string> };
    }>(
      '/partner/verifications/:verificationId/documents/:documentId/details',
      { preHandler: partnerPortalAuth, schema: documentDetailsSchema() },
      async (request, reply) => {
        const partner = requirePartnerPortalContext(request);
        await options.service.updateDocumentDetails!({
          verificationId: request.params.verificationId,
    ...
    fastify.get<{ Params: { verificationId: string } }>(
      ...
      {
        preHandler: partnerPortalAuth,
        schema: { params: verificationParams() },
      },
      async (request) =>
        options.service.listDocuments!({
          verificationId: request.params.verificationId,
          subjectId: requirePartnerPortalContext(request).partnerId,
        }),
    );
  }
  ...
  fastify.patch<{
    ...
    '/owner/verifications/:verificationId/documents/:documentId/details',
    { preHandler: ownerAuth, schema: documentDetailsSchema() },
    async (request, reply) => {
  ...
  fastify.get<{ Params: { verificationId: string } }>(
    ...
    { preHandler: ownerAuth, schema: { params: verificationParams() } },
    async (request) =>
      options.service.listDocuments!({
        verificationId: request.params.verificationId,
        subjectId: requireOwnerContext(request).ownerId,
      }),
  );
  ...
  fastify.patch<{
    Params: { partnerId: string; verificationId: string; documentId: string };
    Body: { documentType: string; details: Record<string, string> };
  }>(
    '/admin/partners/:partnerId/verifications/:verificationId/documents/:documentId/details',
    {
      ...
      schema: {
        ...
        params: {
          type: 'object',
          additionalProperties: false,
          required: ['partnerId', 'verificationId', 'documentId'],
          properties: {
  ...
  fastify.get<{ Params: { partnerId: string; verificationId: string } }>(
    '/admin/partners/:partnerId/verifications/:verificationId/documents',
    {
      ...
      schema: {
        params: {
          type: 'object',
          additionalProperties: false,
          required: ['partnerId', 'verificationId'],
          properties: {
            partnerId: { type: 'string', pattern: '^[a-fA-F0-9]{24}$' },
            verificationId: { type: 'string', pattern: '^[a-fA-F0-9]{24}$' },
          },
    ...
    async (request) => {
      requireAdminRole(request);
      return options.service.listDocuments!({
        verificationId: request.params.verificationId,
        subjectId: request.params.partnerId,
      });
    },
  ...
  fastify.get<{ Params: { ownerId: string; verificationId: string } }>(
    '/admin/owners/:ownerId/verifications/:verificationId/documents',
    {
      ...
      schema: {
        params: {
          type: 'object',
          additionalProperties: false,
          required: ['ownerId', 'verificationId'],
          properties: {
    ...
    async (request) => {
      ...
      return options.service.listDocuments!({
        verificationId: request.params.verificationId,
        subjectId: request.params.ownerId,
      });
    },
  );
  ...
  fastify.post<{
          ...
          properties: {
            ...
            checklist: {
              ...
              properties: {
                ...
                detailsMatch: { type: 'boolean' },
                gstChecked: { type: 'boolean' },
                panChecked: { type: 'boolean' },
                bankChecked: { type: 'boolean' },
                aadhaarMasked: { type: 'boolean' },
              },
            },
    ...
    async (request, reply) => {
      const actor = requireAdminContext(request);
      if (actor.role === 'SUPPORT')
        throw new AppError({
          code: 'KYC_REVIEW_ROLE_REQUIRED',
          message: 'ADMIN or OPS role is required for preliminary KYC review',
          statusCode: 403,
        });
      if (!options.service.preliminaryReview)
        throw new AppError({
          code: 'KYC_PRELIMINARY_REVIEW_UNAVAILABLE',
          message: 'Preliminary KYC review is unavailable',
          statusCode: 503,
        });
      await options.service.preliminaryReview({
        verificationId: request.params.verificationId,
...
function documentDetailsSchema() {
  return {
    params: {
      type: 'object',
      additionalProperties: false,
    ...
    body: {
      ...
      properties: {
        documentType: {
          enum: ['GST_CERTIFICATE', 'PAN', 'PASSBOOK', 'AADHAAR'],
        },
        details: {
          type: 'object',
          minProperties: 2,
