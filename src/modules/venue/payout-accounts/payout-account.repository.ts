...
export interface PayoutAccountRepository {
  ...
  addDocument(input: {
    ...
    accountId: ObjectId;
    expectedVersion: number;
    document: VenuePayoutAccountDocument['documents'][number];
    now: Date;
    actorOwnerId: ObjectId;
  }): Promise<VenuePayoutAccountDocument | null>;
  setDefault(input: {
...
export function createPayoutAccountRepository(
  ...
  return {
    ...
    find(venueId, accountId) {
      return accounts().findOne({ _id: accountId, venue_id: venueId });
    },
    updateDetails(input) {
        ...
        },
        {
          ...
          $push: {
            audit_history: {
              $each: [
                {
                  event_type: 'PAYOUT_ACCOUNT_DETAILS_UPDATED',
        ...
        },
        {
          ...
          $push: {
            audit_history: {
              $each: [
                {
                  event_type: 'PAYOUT_ACCOUNT_DISABLED',
                  actor_type: 'VENUE_OWNER',
                  actor_id: input.actorOwnerId,
                  occurred_at: input.now,
                },
      ...
      return accounts().findOneAndUpdate(
        {
          ...
          version: input.expectedVersion,
          status: { $ne: 'DISABLED' },
          'documents.document_id': { $ne: input.document.document_id },
        },
        {
          $push: {
            ...
            audit_history: {
              $each: [
                {
                  event_type: 'PAYOUT_ACCOUNT_DOCUMENT_ADDED',
                  actor_type: 'VENUE_OWNER',
                  actor_id: input.actorOwnerId,
    ...
    async setDefault(input) {
      let result: VenuePayoutAccountDocument | null = null;
      await database.withTransaction(async ({ session }) => {
        const eligible = await accounts().findOne(
          { _id: input.accountId, venue_id: input.venueId, status: 'VERIFIED' },
          { session },
        );
        if (!eligible) return;
        await accounts().updateMany(
          { venue_id: input.venueId, is_default: true },
          { $set: { is_default: false, updated_at: input.now } },
          { session },
        );
        result = await accounts().findOneAndUpdate(
          { _id: input.accountId, venue_id: input.venueId, status: 'VERIFIED' },
          {
            $set: { is_default: true, updated_at: input.now },
            $inc: { version: 1 },
            $push: {
              audit_history: {
                $each: [
                  {
                    event_type: 'PAYOUT_ACCOUNT_SET_DEFAULT',
                    actor_type: 'VENUE_OWNER',
                    actor_id: input.actorOwnerId,
                    occurred_at: input.now,
                  },
