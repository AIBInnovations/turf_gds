...
> = async (fastify, options) => {
  ...
  fastify.get<{ Params: { venueId: string; accountId: string } }>(
    '/:venueId/payout-accounts/:accountId',
    { preHandler: authenticate, schema: { params: accountParams() } },
    async (request) => {
      const owner = requireOwnerContext(request);
    ...
    '/:venueId/payout-accounts/:accountId',
    {
      ...
      schema: {
        ...
        body: {
          type: 'object',
          additionalProperties: false,
          required: ['version', 'accountHolderName', 'bankName', 'ifscCode'],
          properties: {
  ...
  fastify.delete<{
    ...
    async (request) => {
      const owner = requireOwnerContext(request);
      return options.service.disable({
        actorOwnerId: owner.ownerId,
        ...request.params,
        ...request.query,
      });
  ...
  fastify.post<{ Params: { venueId: string; accountId: string } }>(
    '/:venueId/payout-accounts/:accountId/default',
    { preHandler: authenticate, schema: { params: accountParams() } },
    async (request) => {
      const owner = requireOwnerContext(request);
      return options.service.setDefault({
        actorOwnerId: owner.ownerId,
        ...request.params,
      });
    },
  ...
  fastify.post<{
    ...
    '/:venueId/payout-accounts/:accountId/documents',
    {
      preHandler: authenticate,
      schema: {
        params: accountParams(),
...
function accountParams() {
  return {
    ...
    properties: {
      venueId: { type: 'string', pattern: '^[a-fA-F0-9]{24}$' },
      accountId: { type: 'string', pattern: '^[a-fA-F0-9]{24}$' },
    },
  } as const;
}
