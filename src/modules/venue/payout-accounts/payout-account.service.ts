...
const PAYOUT_DOCUMENT_MIME_TYPES = [
  'application/pdf',
  'image/jpeg',
  'image/png',
  'image/webp',
...
export interface PayoutAccountService {
  ...
  list(input: { actorOwnerId: string; venueId: string }): Promise<object[]>;
  get(input: {
    actorOwnerId: string;
    venueId: string;
    accountId: string;
...
export function createPayoutAccountService(input: {
  ...
  return {
    ...
    async list(values) {
      ...
      return (await input.repository.list(oid(values.venueId))).map((value) =>
        present(value),
      );
    },
    async get(values) {
      ...
      const value = await input.repository.find(
        oid(values.venueId),
        oid(values.accountId),
      );
    ...
    async update(values) {
      ...
      const ifsc = values.ifscCode.toUpperCase();
      if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(ifsc))
        throw invalid('INVALID_PAYOUT_ACCOUNT', 'IFSC code is invalid');
      const value = await input.repository.updateDetails({
        ...
        bankName: required(values.bankName, 'bankName'),
        ifscCode: ifsc,
        now: now(),
        actorOwnerId: oid(values.actorOwnerId),
      });
      if (!value) throw versionConflict();
      return present(value, true);
    },
    async disable(values) {
      await input.ownerAccessService.requirePermission(
        values.actorOwnerId,
        values.venueId,
        'MANAGE_VENUE',
      );
      const value = await input.repository.disable({
        venueId: oid(values.venueId),
        accountId: oid(values.accountId),
        expectedVersion: values.version,
        now: now(),
    ...
    async setDefault(values) {
      ...
      const value = await input.repository.setDefault({
        venueId: oid(values.venueId),
        accountId: oid(values.accountId),
        now: now(),
        actorOwnerId: oid(values.actorOwnerId),
      ...
      if (!value)
        throw new AppError({
          ...
          message: 'Only a verified payout account can be the default',
          statusCode: 409,
        });
      return present(value, true);
    },
    async uploadDocument(values) {
      ...
      if (
        !PAYOUT_DOCUMENT_MIME_TYPES.includes(mimeType) ||
        values.buffer.length === 0 ||
        values.buffer.length > 10 * 1024 * 1024
      )
        throw invalid(
          'UNSUPPORTED_PAYOUT_DOCUMENT',
          'Payout document must be PDF, JPEG, PNG, or WebP up to 10 MB',
        );
      inspectUploadedFile(values.buffer, mimeType, PAYOUT_DOCUMENT_MIME_TYPES);
      const uploaded = await input.mediaStorage.uploadBuffer(values.buffer, {
        folder: `turf-gds/payout-accounts/${values.venueId}`,
        access: 'authenticated',
        resourceType: 'auto',
        tags: ['payout-account', values.documentType],
      });
      const timestamp = now();
      try {
        const value = await input.repository.addDocument({
          venueId: oid(values.venueId),
          accountId: oid(values.accountId),
          expectedVersion: values.version,
          actorOwnerId: oid(values.actorOwnerId),
...
function present(value: VenuePayoutAccountDocument, detail = false) {
  return {
    ...
    ...(detail
      ? {
          verificationFailureReason: value.verification_failure_reason,
          documents: value.documents.map((document) => ({
            documentId: document.document_id.toHexString(),
...
function versionConflict() {
  return conflict(
    'PAYOUT_ACCOUNT_VERSION_CONFLICT',
    'Payout account changed concurrently or is disabled',
  );
}
