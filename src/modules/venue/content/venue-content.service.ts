...
export function createVenueContentService(input: {
  ...
  return {
    async get(values) {
      ...
      return value
        ? present(value)
        : { venueId: values.venueId, locale, content: {}, version: 0 };
    },
    async put(values) {
      ...
      if (
        existing &&
        (!Number.isInteger(values.version) ||
          values.version !== existing.version)
      ) {
        throw conflict();
      }
      if (!existing && values.version !== undefined && values.version !== 0)
        throw conflict();
      const timestamp = now();
...
function validateContent(content: Record<string, unknown>): void {
  ...
  if (bytes > 256 * 1024)
    throw new AppError({
      code: 'VENUE_CONTENT_TOO_LARGE',
      message: 'Venue content must not exceed 256 KiB',
      statusCode: 413,
    });
  for (const key of Object.keys(content)) {
    if (!/^[A-Za-z][A-Za-z0-9_-]{0,63}$/.test(key)) {
      throw new AppError({
        code: 'INVALID_VENUE_CONTENT_KEY',
        message: `Invalid venue content key: ${key}`,
        statusCode: 400,
      });
    }
  }
...
function normalizeLocale(value = 'en-IN') {
...
function conflict() {
  return new AppError({
    code: 'VENUE_CONTENT_VERSION_CONFLICT',
    message: 'Venue content changed concurrently',
    statusCode: 409,
  });
}
