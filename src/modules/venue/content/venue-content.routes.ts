...
export interface VenueContentRoutesOptions {
  service: VenueContentService;
...
const query = {
  type: 'object',
  additionalProperties: false,
  properties: { locale: { type: 'string', minLength: 2, maxLength: 35 } },
} as const;

const routes: FastifyPluginAsync<VenueContentRoutesOptions> = async (
  fastify,
  options,
) => {
  ...
  fastify.get<{
    ...
    '/:venueId/content',
    { preHandler: auth, schema: { params, querystring: query } },
    async (request) => {
