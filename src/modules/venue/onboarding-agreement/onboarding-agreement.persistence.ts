...
const validator: Document = {
  $jsonSchema: {
    ...
    properties: {
      ...
      template_version: { bsonType: ['int', 'null'], minimum: 1 },
      terms_hash: { bsonType: 'string' },
      platform_commission_bps: bps,
      settlement_cycle: { enum: ['T_PLUS_N', 'WEEKLY', 'MONTHLY'] },
      settlement_lag_days: { bsonType: 'int', minimum: 0 },
      status: {
        enum: ['PROPOSED', 'CHANGES_REQUESTED', 'ACCEPTED', 'SUPERSEDED'],
      },
...
export async function initializeOnboardingAgreementPersistence(db: Db) {
  ...
  if (exists) {
    ...
    const cursor = db
      .collection<{ _id: import('mongodb').ObjectId; terms_text: string }>(name)
      .find({ terms_hash: { $exists: false } });
  ...
  const templateValidator: Document = {
    $jsonSchema: {
      bsonType: 'object',
      additionalProperties: false,
      required: [
  ...
  if (
    !(await db
      .listCollections({ name: templateName }, { nameOnly: true })
      .hasNext())
  )
    await db.createCollection(templateName, {
  ...
  await db
    ...
    .createIndex(
      { code: 1, version: 1 },
      { unique: true, name: 'uq_contract_template_version' },
    );
}
