    ...
    async get(values) {
      ...
      if (start >= end || end.getTime() - start.getTime() > 31 * 86_400_000) {
        throw new AppError({
          ...
          message:
            'Dashboard range must be positive and no longer than 31 days',
          statusCode: 400,
        });
      }
      const db = input.database.db;
      const [bookings, upcoming, courts, slots, payouts, owner] =
        await Promise.all([
          db
            .collection<BookingDocument>('bookings')
            .find({ venue_id: venueId, starts_at: { $gte: start, $lt: end } })
            .toArray(),
          db
            .collection<BookingDocument>('bookings')
            .find({
      ...
      const occupied = slots.filter((value) =>
        ['HELD', 'BOOKED', 'BLOCKED', 'UNAVAILABLE'].includes(value.status),
      ).length;
      const notifications = (owner?.notifications ?? []) as Array<{
        venue_id?: ObjectId;
        read_at?: Date | null;
      }>;
      return {
        ...
        bookings: {
          ...
          cancelled: bookings.filter((value) => value.status === 'CANCELLED')
            .length,
          direct: bookings.filter((value) => value.booking_type === 'DIRECT')
            .length,
          grossAmountMinor: confirmed.reduce(
            (sum, value) => sum + value.gross_amount_minor,
            0,
          ),
          venueNetAmountMinor: confirmed.reduce(
            (sum, value) => sum + value.venue_net_amount_minor,
            0,
          ),
          currency: 'INR',
        },
        inventory: {
          ...
          occupied,
          total: slots.length,
          occupancyPercent: slots.length
            ? Math.round((occupied / slots.length) * 10_000) / 100
            : 0,
        },
        courts: {
          total: courts.length,
          available: courts.filter((value) => value.status === 'AVAILABLE')
            .length,
          unavailable: courts.filter((value) => value.status === 'UNAVAILABLE')
            .length,
        },
        notifications: {
          unread: notifications.filter(
            (value) => value.venue_id?.equals(venueId) && !value.read_at,
          ).length,
        },
        upcomingBookings: upcoming.map((value) => ({
        ...
        recentPayouts: payouts.map((value) => ({
...
function instant(value: string) {
  ...
  if (Number.isNaN(result.getTime()))
    throw new AppError({
      code: 'INVALID_DATE',
      message: 'Dashboard dates must be ISO date-times',
      statusCode: 400,
    });
  return result;
}
