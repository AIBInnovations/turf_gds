...
const courtProperties = {
  ...
  fixedSlotAnchorMinutes: {
    anyOf: [{ type: 'integer', minimum: 0, maximum: 1439 }, { type: 'null' }],
  },
} as const;

const courtOwnerRoutes: FastifyPluginAsync<CourtOwnerRoutesOptions> = async (
  fastify,
  options,
) => {
  ...
  fastify.post<{
    ...
    Body: Omit<CreateCourtInput, 'actorOwnerId' | 'venueId' | 'correlationId'>;
  }>(
    '/:venueId/courts',
    {
