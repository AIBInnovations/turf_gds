...
export function createAdminVenueService(input: {
  ...
  return {
    ...
    async getVenue(venueId) {
      ...
      return {
        ...presentVenue(venue),
        ownerId: owner?.owner_id?.toHexString() ?? null,
      };
    },
    async createVenue(values) {
      ...
      await input.database.withTransaction(async ({ session }) => {
        ...
        if (!input.identityService.attachOwnerVenue) {
          throw new Error(
            'Identity owner attachment capability is unavailable',
          );
        }
        membershipId = (
          await input.identityService.attachOwnerVenue(
            {
    ...
    async updateVenue(values) {
      ...
      if (values.legalName !== undefined)
        changes.legal_name = required(values.legalName, 'legalName');
      if (values.displayName !== undefined)
        changes.display_name = required(values.displayName, 'displayName');
      if (values.timezone !== undefined)
        changes.timezone = required(values.timezone, 'timezone');
      if (values.status !== undefined && values.status !== existing.status) {
        if (existing.status === 'PENDING' || values.status === 'PENDING') {
          throw conflict(
            'VENUE_STATUS_TRANSITION_NOT_ALLOWED',
            'Pending Venue activation must use onboarding approval',
          );
        }
        required(values.reason, 'reason');
        if (existing.status === 'SUSPENDED' && values.status === 'ACTIVE') {
          const membership = await input.database.db
            .collection('venue_owner_memberships')
          ...
          if (!verification)
            throw conflict(
              'VERIFIED_OWNER_KYC_REQUIRED',
              'Current verified Owner BUSINESS KYC is required',
            );
        }
      ...
      if (fields.length === 0)
        throw bad('EMPTY_UPDATE', 'At least one change is required');
      const timestamp = now();
      ...
      if (!updated)
        throw conflict('VENUE_VERSION_CONFLICT', 'Venue version is stale');
      return presentVenue(updated);
    },
    async listCourts(venueId) {
      const id = oid(venueId);
      if (!(await venues().findOne({ _id: id })))
        throw notFound('VENUE_NOT_FOUND', 'Venue not found');
      return (
        await courts()
          .find({ venue_id: id })
          .sort({ name: 1, _id: 1 })
    ...
    async createCourt(values) {
      const venueId = oid(values.venueId);
      if (!(await venues().findOne({ _id: venueId })))
        throw notFound('VENUE_NOT_FOUND', 'Venue not found');
      validateDurations(
        values.minBookingMinutes,
        values.bookingIncrementMinutes,
      );
      const timestamp = now();
      ...
      try {
        ...
        if (isDuplicate(error))
          throw conflict(
            'COURT_NAME_EXISTS',
            'Court name already exists for this Venue',
          );
        throw error;
      }
      return presentCourt(court);
    ...
    async updateCourt(values) {
      ...
      if (values.name !== undefined)
        changes.name = required(values.name, 'name');
      if (values.surfaceType !== undefined)
        changes.surface_type = required(values.surfaceType, 'surfaceType');
      if (values.capacity !== undefined)
        changes.capacity = positive(values.capacity, 'capacity');
      ...
      if (!updated) {
        if (!(await courts().findOne({ _id: courtId, venue_id: venueId })))
          throw notFound('COURT_NOT_FOUND', 'Court not found');
...
function presentCourt(value: CourtDocument) {
  return {
    ...
    operatingHours: value.operating_hours.entries,
    version: value.version,
    auditHistory: value.audit_history,
  };
}
...
function required(value: string | undefined, name: string) {
  ...
  if (!result) throw bad('INVALID_INPUT', `${name} is required`);
  return result;
}
function positive(value: number, name: string) {
  if (!Number.isInteger(value) || value < 1)
    throw bad('INVALID_INPUT', `${name} must be a positive integer`);
  return value;
}
function validateDurations(min: number, increment: number) {
  if (
    ...
    throw bad('INVALID_COURT_DURATION', 'Court duration values are invalid');
}
