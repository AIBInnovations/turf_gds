...
export interface OwnerBookingService {
  ...
  recordPayment(input: {
    ...
    bookingId: string;
    amountMinor: number;
    method: BookingPaymentDocument['method'];
    reference?: string;
    notes?: string;
    correlationId: string;
  }): Promise<ReturnType<typeof paymentView>>;
  refundPayment(input: {
    actorOwnerId: string;
    venueId: string;
    bookingId: string;
...
export function createOwnerBookingService(input: {
  ...
  return {
    ...
    async getDetail(values) {
      ...
      return {
        ...toDetailView(booking, cancellation),
    ...
    async createDirectBooking(values) {
      ...
      if ((endsAt.getTime() - startsAt.getTime()) / 60_000 < 60) {
        throw new AppError({
          ...
          message: 'Direct bookings must be at least 60 minutes',
          statusCode: 400,
        });
      }
      ...
      await input.database.withTransaction(async ({ session }) => {
        ...
        if (
          court.booking_mode !== 'BOTH' &&
          court.booking_mode !== 'OPEN_TIME'
        ) {
          throw new AppError({
            code: 'COURT_BOOKING_MODE_NOT_ALLOWED',
            message: 'Court does not allow DIRECT bookings',
            statusCode: 409,
          });
        }
        ...
        if (overlap.blocking.length > 0) {
          throw new AppError({
            code: 'INVENTORY_OVERLAP',
            message: 'The interval overlaps unavailable inventory',
            statusCode: 409,
          });
        }
        ...
        await input.repository.insertDirectSlot(slot, session);
    ...
    async cancelBooking(values) {
      ...
      await input.database.withTransaction(async ({ session }) => {
        const booking = await input.repository.findForVenueWithSession(
          venueId,
          bookingId,
          session,
        );
        ...
        if (booking.booking_type !== 'DIRECT') {
          throw new AppError({
            ...
            message:
              'This endpoint only cancels bookings created by the Venue Owner',
            statusCode: 409,
          });
        }
        ...
        if (!result) {
          throw new AppError({
            ...
            message: 'Booking changed concurrently',
            statusCode: 409,
          });
        }
        cancelled = result;
        const payment = await input.database.db
          .collection<BookingPaymentDocument>('booking_payments')
          .findOne({ booking_id: bookingId, venue_id: venueId }, { session });
        const refundAmount = payment
          ? payment.amount_minor - payment.refunded_amount_minor
          : 0;
        if (payment && refundAmount > 0) {
          const refundedPayment = await input.database.db
            .collection<BookingPaymentDocument>('booking_payments')
            .findOneAndUpdate(
          ...
          if (!refundedPayment)
            throw new AppError({
              ...
              message: 'Payment changed concurrently',
              statusCode: 409,
            });
          await input.outboxRepository.enqueue({
    ...
    async recordPayment(values) {
      ...
      if (!Number.isSafeInteger(values.amountMinor) || values.amountMinor < 0)
        throw new AppError({
      ...
      const venueId = toObjectId(values.venueId);
      const bookingId = toObjectId(values.bookingId);
      const ownerId = toObjectId(values.actorOwnerId);
      const timestamp = now();
      let payment!: BookingPaymentDocument;
      await input.database.withTransaction(async ({ session }) => {
        const booking = await input.repository.findForVenueWithSession(
          venueId,
          bookingId,
        ...
        if (!booking || booking.booking_type !== 'DIRECT')
          throw new AppError({
            code: 'DIRECT_BOOKING_NOT_FOUND',
            message: 'Direct booking was not found',
            statusCode: 404,
          });
        if (booking.status !== 'CONFIRMED')
          throw new AppError({
            code: 'PAYMENT_BOOKING_NOT_CONFIRMED',
            message:
              'Payment can only be recorded for a confirmed direct booking',
            statusCode: 409,
          });
        payment = {
          _id: new ObjectId(),
        ...
        await input.database.db
          ...
          .updateOne(
            { _id: bookingId, venue_id: venueId, version: booking.version },
            {
              $set: {
                gross_amount_minor: values.amountMinor,
                venue_net_amount_minor: values.amountMinor,
                partner_payment_reference: payment.reference,
                updated_at: timestamp,
              },
              $inc: { version: 1 },
            },
    ...
    async refundPayment(values) {
      ...
      const venueId = toObjectId(values.venueId),
        bookingId = toObjectId(values.bookingId),
        ownerId = toObjectId(values.actorOwnerId),
        timestamp = now();
      let payment!: BookingPaymentDocument;
      await input.database.withTransaction(async ({ session }) => {
        ...
        if (!existing)
          throw new AppError({
            ...
            message: 'Booking payment was not found',
            statusCode: 404,
          });
        if (
          existing.version !== values.version ||
          existing.refunded_amount_minor + values.amountMinor >
            existing.amount_minor
        )
          throw new AppError({
            code: 'PAYMENT_REFUND_CONFLICT',
            message:
              'Payment changed or refund exceeds the remaining paid amount',
            statusCode: 409,
          });
        const refunded = existing.refunded_amount_minor + values.amountMinor;
        payment = (await input.database.db
          .collection<BookingPaymentDocument>('booking_payments')
          .findOneAndUpdate(
            { _id: existing._id, version: values.version },
            {
              $set: {
                refunded_amount_minor: refunded,
                status:
                  refunded === existing.amount_minor
                    ? 'REFUNDED'
                    : 'PARTIALLY_REFUNDED',
                refunded_at: timestamp,
                updated_at: timestamp,
        ...
        await input.outboxRepository.enqueue({
          ...
          correlationId: values.correlationId,
          payload: {
            paymentId: existing._id.toHexString(),
            bookingId: bookingId.toHexString(),
            refundAmountMinor: values.amountMinor,
            totalRefundedAmountMinor: refunded,
            currency: 'INR',
          },
          now: timestamp,
          session,
...
function paymentView(value: BookingPaymentDocument) {
  return {
    paymentId: value._id.toHexString(),
    bookingId: value.booking_id.toHexString(),
    amountMinor: value.amount_minor,
    refundedAmountMinor: value.refunded_amount_minor,
    currency: value.currency,
    method: value.method,
    status: value.status,
    reference: value.reference,
    notes: value.notes,
    paidAt: value.paid_at.toISOString(),
    refundedAt: value.refunded_at?.toISOString() ?? null,
...
function toDate(value: string): Date {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new AppError({
      ...
      message: 'Booking date must be a valid ISO-8601 timestamp',
      statusCode: 400,
    });
  }
  return date;
