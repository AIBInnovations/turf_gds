...
const ownerBookingRoutes: FastifyPluginAsync<
  OwnerBookingRoutesOptions
> = async (fastify, options) => {
  ...
  fastify.post<{
    ...
    '/:venueId/bookings/:bookingId/payment',
    {
      preHandler: authenticate,
      schema: {
        params: {
    ...
    async (request, reply) => {
      ...
      return reply
        .status(201)
        .send(
          await options.service.recordPayment({
            actorOwnerId: owner.ownerId,
            venueId: request.params.venueId,
            bookingId: request.params.bookingId,
            correlationId: request.id,
  ...
  fastify.post<{
    Params: { venueId: string; bookingId: string };
    Body: { amountMinor: number; version: number; notes?: string };
  }>(
    '/:venueId/bookings/:bookingId/payment/refund',
    {
      preHandler: authenticate,
      schema: {
        params: {
    ...
    async (request) => {
      const owner = requireOwnerContext(request);
      return options.service.refundPayment({
        actorOwnerId: owner.ownerId,
        venueId: request.params.venueId,
        bookingId: request.params.bookingId,
        correlationId: request.id,
