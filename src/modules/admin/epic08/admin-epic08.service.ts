...
export interface AdminEpic08Service {
  ...
  revenueReport(
    input: ReportFilter & { groupBy?: 'DAY' | Dimension },
  ): Promise<Record<string, unknown>>;
  activityReport(
    input: ReportFilter & { dimension: Dimension },
  ): Promise<Record<string, unknown>>;
  exportReport(
    kind: 'bookings' | 'revenue' | 'activity',
    input: ReportFilter & {
  ...
  resolveDispute(input: {
    ...
    environment: Environment;
    expectedVersion: number;
    resolution: 'CONFIRMED' | 'CANCELLED' | 'REFUNDED';
    note: string;
    adminId: string;
    correlationId: string;
  }): Promise<Record<string, unknown>>;
...
export function createAdminEpic08Service(input: {
        ...
        .aggregate([
          ...
          },
          {
            $project: {
              ...
              venueId: { $toString: '$venue_id' },
              partnerId: { $toString: '$partner_id' },
              courtId: { $toString: '$court_id' },
              ...
              venueNetAmountMinor: '$venue_net_amount_minor',
              currency: 1,
            },
          },
        ])
  ...
  async function revenueReport(
    values: ReportFilter & { groupBy?: 'DAY' | Dimension },
  ) {
    validateRange(values);
    ...
    const groupExpression =
      values.groupBy === 'DAY'
        ...
        : values.groupBy === 'VENUE'
          ? '$venue_id'
          : values.groupBy === 'PARTNER'
            ? '$partner_id'
            : null;
    ...
    pipeline.push(
      {
        $set: {
          component: {
            $ifNull: ['$metadata.component', '$metadata.original_component'],
          },
          signed: {
            $cond: [
              { $eq: ['$direction', 'CREDIT'] },
      ...
      },
      {
        $group: {
          ...
          commissionAmountMinor: {
            $sum: {
              $cond: [{ $eq: ['$component', 'COMMISSION'] }, '$signed', 0],
            },
          },
          taxAmountMinor: {
            $sum: { $cond: [{ $eq: ['$component', 'TAX'] }, '$signed', 0] },
          },
          venueNetAmountMinor: {
            $sum: {
              $cond: [{ $eq: ['$component', 'VENUE_NET'] }, '$signed', 0],
            },
          },
          refundAmountMinor: {
            $sum: {
              $cond: [{ $eq: ['$entry_type', 'REVERSAL'] }, '$amount_minor', 0],
            },
          },
          adjustmentAmountMinor: {
            $sum: {
              $cond: [{ $eq: ['$entry_type', 'ADJUSTMENT'] }, '$signed', 0],
            },
          },
          entryCount: { $sum: 1 },
        },
    ...
    const settlementMatch = {
      environment: values.environment,
      period_start: { $lt: values.to },
      period_end: { $gt: values.from },
      ...(values.partnerId ? { partner_id: oid(values.partnerId) } : {}),
    };
        ...
        .aggregate([
          { $match: settlementMatch },
          {
            $group: {
              _id: '$status',
              count: { $sum: 1 },
              amountMinor: { $sum: '$net_amount_minor' },
            },
          },
          { $sort: { _id: 1 } },
        ])
        ...
        .aggregate([
          {
            $match: {
              environment: values.environment,
              created_at: { $gte: values.from, $lt: values.to },
              ...(values.venueId ? { venue_id: oid(values.venueId) } : {}),
            },
          },
          {
    ...
    return {
      ...
      buckets: buckets.map((value) => ({
        ...value,
        key: value._id?.toHexString?.() ?? value._id,
        _id: undefined,
      })),
      settlements,
      payouts,
  ...
  async function activityReport(
    values: ReportFilter & { dimension: Dimension },
  ) {
    validateRange(values);
    const match = bookingMatch(values);
      ...
      .aggregate([
        ...
        },
        {
          $project: {
            ...
            name: { $arrayElemAt: ['$subject.display_name', 0] },
            bookingCount: 1,
            confirmedCount: 1,
            cancelledCount: 1,
            grossAmountMinor: 1,
  ...
  return {
        ...
        for (let page = 1; page <= 100; page += 1) {
          ...
          if (result.total > 10_000) {
            throw new AppError({
              ...
              message: 'CSV export exceeds 10000 rows',
              statusCode: 413,
            });
          }
    ...
    async dispute(bookingId, environment) {
      ...
      const booking = await db
        .collection('bookings')
        .findOne({ _id: id, environment });
      if (!booking) throw notFound('BOOKING_NOT_FOUND', 'Booking not found');
      const [cancellation, venue, court, partner, slot, ledger, outbox] =
        await Promise.all([
          ...
          db
            .collection('ledger_entries')
            .find({ booking_id: id, environment })
            .sort({ effective_at: 1, _id: 1 })
            .toArray(),
          db
            .collection('outbox_events')
            .find({ aggregate_type: 'BOOKING', aggregate_id: id, environment })
            .project({ payload: 0, webhook_endpoint_ids: 0 })
            .toArray(),
        ]);
      const settlementIds = [
        ...new Map(
          ledger
            .filter((entry) => entry.settlement_id)
            .map((entry) => [
              entry.settlement_id.toHexString(),
              entry.settlement_id,
            ]),
        ).values(),
      ];
      const settlements = settlementIds.length
        ? await db
            .collection('settlements')
            .find({ _id: { $in: settlementIds }, environment })
            .toArray()
        : [];
      const [reconciliations, payouts] = await Promise.all([
        settlementIds.length
          ? db
              ...
              .find({ settlement_id: { $in: settlementIds }, environment })
              .toArray()
          : [],
        settlementIds.length
          ? db
    ...
    async addDisputeNote(values) {
      ...
      if (!note || note.length > 2_000)
        throw bad(
          'INVALID_DISPUTE_NOTE',
          'note must contain 1 to 2000 characters',
        );
      const result = await db
        .collection<BookingDocument>('bookings')
        .findOneAndUpdate(
          {
            _id: oid(values.bookingId),
            environment: values.environment,
    ...
    async inventoryHealth(values) {
      ...
      const threshold = new Date(
        current.getTime() + input.minimumCoverageDays * 86_400_000,
      );
      const match: Document = {
        ...(values.environment ? { environment: values.environment } : {}),
        ...(values.venueId ? { _id: oid(values.venueId) } : {}),
      };
      ...
      for (const venue of venues) {
        ...
        for (const court of venueCourts) {
          const slots = await db
            ...
            .toArray();
          const fixed = slots.filter(
            (slot) => slot.booking_type === 'FIXED_SLOT',
          );
          const coverageThrough = fixed.reduce<Date | null>(
            (max, slot) => (!max || slot.ends_at > max ? slot.ends_at : max),
            null,
          );
          const counts = Object.fromEntries(
            ['AVAILABLE', 'HELD', 'BOOKED', 'BLOCKED', 'UNAVAILABLE'].map(
              (status) => [
                status,
          ...
          const activeOpenTimeBlocks = slots.filter(
            (slot) =>
              slot.booking_type === 'OPEN_TIME' && slot.status === 'BLOCKED',
          ).length;
          const expiredHolds = await db
            ...
            .countDocuments({
              ...
              environment: venue.environment,
              status: 'HELD',
              hold_expires_at: { $lte: current },
            });
          const health =
            venue.status !== 'ACTIVE' || court.status !== 'AVAILABLE'
              ? 'DISABLED'
              : fixed.length === 0 &&
                  ['FIXED_SLOT', 'BOTH'].includes(court.booking_mode)
                ? 'EMPTY'
                : ['FIXED_SLOT', 'BOTH'].includes(court.booking_mode) &&
                    (!coverageThrough || coverageThrough < threshold)
                  ? 'STALE'
                  : 'HEALTHY';
          rows.push({
            ...
            health,
            coverageThrough,
            latestGeneratedAt: fixed.reduce<Date | null>(
              (max, slot) =>
                !max || slot.created_at > max ? slot.created_at : max,
              null,
            ),
            statusCounts: counts,
            activeOpenTimeBlocks,
            expiredHolds,
          });
        }
      }
      const filtered = values.health
        ? rows.filter(({ health }) => health === values.health)
        : rows;
      return {
        ...
        minimumCoverageDays: input.minimumCoverageDays,
      };
    },
    async listPartners(values) {
      const { page, limit } = pagination(values.page, values.limit);
      const match: Document = {
        ...
        ...(values.q
          ? {
              $or: [
                {
                  legal_name: { $regex: escapeRegex(values.q), $options: 'i' },
                },
                {
                  display_name: {
                    $regex: escapeRegex(values.q),
                    $options: 'i',
                  },
                },
                { email: { $regex: escapeRegex(values.q), $options: 'i' } },
              ],
            }
          : {}),
      };
      const total = await db.collection('partners').countDocuments(match);
      const items = await db
        .collection('partners')
        .find(match)
        .project({ password_hash: 0, sessions: 0, audit_history: 0 })
        .sort({ created_at: -1 })
        .skip((page - 1) * limit)
        .limit(limit)
        .toArray();
    ...
    async partnerDetail(partnerId) {
      const id = oid(partnerId);
      const [partner, kyc, keys, webhooks, payoutAccounts] = await Promise.all([
        db
          .collection('partners')
          .findOne(
            { _id: id },
            { projection: { password_hash: 0, sessions: 0 } },
          ),
        db
          .collection('kyc_verifications')
          .find({ subject_type: 'PARTNER', subject_id: id })
          .sort({ created_at: -1 })
          .toArray(),
        db
          .collection('partner_api_keys')
          .find({ partner_id: id })
          .project({ key_hash: 0, signing_secret_hash: 0 })
          .toArray(),
        db
          .collection('webhook_endpoints')
          .find({ partner_id: id })
          .project({ signing_secret_hash: 0 })
          .toArray(),
        db
          .collection('partner_payout_accounts')
          .find({ partner_id: id })
          .project({ account_vault_token: 0, 'documents.secure_url': 0 })
          .toArray(),
      ]);
      if (!partner) throw notFound('PARTNER_NOT_FOUND', 'Partner not found');
      return { partner, kyc, keys, webhooks, payoutAccounts };
    },
    async partnerApiUsage(values) {
      const { page, limit } = pagination(values.page, values.limit);
      const match: Document = {
        ...(values.partnerId ? { partner_id: oid(values.partnerId) } : {}),
        ...(values.environment ? { environment: values.environment } : {}),
        ...(values.from || values.to
          ? {
              usage_date: {
                ...(values.from ? { $gte: values.from } : {}),
                ...(values.to ? { $lt: values.to } : {}),
              },
            }
          : {}),
      };
    ...
    async resolveDispute(values) {
      ...
      const result = await db
        ...
        .findOneAndUpdate(
          ...
          {
            $set: { status: values.resolution, updated_at: now() },
            $inc: { version: 1 },
            $push: {
              audit_history: {
                $each: [
                  {
                    ...
                    actor_id: oid(values.adminId),
                    correlation_id: values.correlationId,
                    changes: { resolution: values.resolution, note },
                    occurred_at: now(),
                  },
                ],
                $slice: -100,
      ...
      if (!result)
        throw new AppError({
          ...
          message: 'The disputed booking changed or was already resolved',
          statusCode: 409,
        });
      return {
        bookingId: result._id.toHexString(),
    ...
    async operationsHealth() {
      ...
      const [
        ...
        db
          ...
          .countDocuments({
            status: 'HELD',
            hold_expires_at: { $lte: timestamp },
          }),
        db.collection('settlements').countDocuments({ status: 'FAILED' }),
        db.collection('payouts').countDocuments({ status: 'FAILED' }),
        db
          .collection('kyc_verifications')
      ...
      return {
        status:
          failedEvents + failedSettlements + failedPayouts > 0
            ? 'DEGRADED'
            : 'HEALTHY',
...
function validateRange(values: ReportFilter) {
  if (
    ...
    Number.isNaN(values.from.valueOf()) ||
    !(values.to instanceof Date) ||
    Number.isNaN(values.to.valueOf()) ||
    values.from >= values.to
  )
    throw bad('INVALID_DATE_RANGE', 'from must be earlier than to');
  if (values.to.valueOf() - values.from.valueOf() > 366 * 86_400_000)
    throw bad('REPORT_RANGE_TOO_LARGE', 'Report range cannot exceed 366 days');
}
function bookingMatch(values: ReportFilter): Document {
  return {
    environment: values.environment,
    starts_at: { $gte: values.from, $lt: values.to },
    ...(values.venueId ? { venue_id: oid(values.venueId) } : {}),
    ...(values.partnerId ? { partner_id: oid(values.partnerId) } : {}),
    ...(values.status ? { status: values.status } : {}),
  };
...
function pagination(page = 1, limit = 20) {
  if (
    ...
    throw bad('INVALID_PAGINATION', 'page and limit are out of range');
  return { page, limit };
}
function oid(value: string) {
  if (!ObjectId.isValid(value))
    throw bad('INVALID_ID', 'A supplied identifier is invalid');
...
function emptyTotals() {
  return {
    ...
    taxAmountMinor: 0,
    venueNetAmountMinor: 0,
  };
}
function summary(value: Document | null) {
  if (!value) return null;
  return {
    id: value._id.toHexString(),
    displayName: value.display_name ?? value.name ?? null,
